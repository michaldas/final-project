app.controller("meetingsGalleryCtrl", function ($scope, meetings, $http, Meet) {
        ...
        $http.get("data/meet.json").then(function (response) {
            ...
            for (var i = 0; i < response.data.length; i++) {
                $scope.meetings.push(new Meet(response.data[i].title, response.data[i].date, response.data[i].time, response.data[i].location, response.data[i].createdBy, response.data[i].tasks, response.data[i].costs, response.data[i].inviteds));
            }
    })
}) 